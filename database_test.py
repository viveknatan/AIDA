...
class DatabaseManager:
    def __init__(self):
        ...
        try:
            ...
            # DON'T add search_path to connection string for Neon
            # We'll set it after connecting instead
            
            self.engine = create_engine(db_url)
            self.schema_name = "northwind"
            
            # Test the connection and set schema after connecting
            with self.engine.connect() as conn:
                ...
                if not result.fetchone():
                    print("⚠️ Northwind schema not found, checking available schemas...")
                    result = conn.execute(text("""
                        SELECT schema_name 
                        FROM information_schema.schemata 
                    ...
                    if schemas:
                        print(f"🔄 Using schema '{schemas[0]}' instead of 'northwind'")
                        self.schema_name = schemas[0]
                    else:
                        print("⚠️ No custom schemas found, using public schema")
                        self.schema_name = "public"
                else:
                    print(f"✅ Connected to PostgreSQL with Northwind schema!")
